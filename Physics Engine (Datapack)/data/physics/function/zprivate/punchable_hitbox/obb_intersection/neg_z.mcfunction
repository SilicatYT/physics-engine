# Calculate t
# (Formula): (Boundary.<Min/Max>.z - Ray.Local.RelativePos.z) / Ray.Local.Direction.z
# (Note): 'Min' boundary for the face that points toward negative z, 'Max' for the other.
# (Note): Scaled up by 2^10, same as EntityInteractionRange and MinDistance for easy comparison.
execute store result score #Physics.Math.t Physics run compute default integer physics:punchable_hitbox/obb_intersection/z/neg_t
...
execute store result score #Physics.Ray.IntersectionPoint.x Physics run compute default integer physics:punchable_hitbox/obb_intersection/intersection_x
execute unless score #Physics.Ray.IntersectionPoint.x Physics matches -32768..32768 run return 0
