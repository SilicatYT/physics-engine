# Calculate t
# (Formula): (AabbRelative.<Min/Max>.x - Ray.RelativePos.x) / Ray.Direction.x
# (Note): 'Min' Aabb for the face that points toward negative x, 'Max' for the other.
# (Note): Scaled up by 2^10, same as EntityInteractionRange and MinDistance for easy comparison.
execute store result score #Physics.Math.t Physics run compute default integer physics:punchable_hitbox/aabb_intersection/x/neg_t
