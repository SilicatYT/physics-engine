# Calculate t
# (Formula): (Boundary.<Min/Max>.y - Ray.Local.RelativePos.y) / Ray.Local.Direction.y
# (Note): 'Min' boundary for the face that points toward negative y, 'Max' for the other.
# (Note): Scaled up by 2^10, same as EntityInteractionRange and MinDistance for easy comparison.
execute store result score #Physics.Math.t Physics run compute default integer physics:punchable_hitbox/obb_intersection/y/pos_t
