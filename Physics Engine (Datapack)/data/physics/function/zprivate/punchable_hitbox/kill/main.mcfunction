...
scoreboard players operation #Physics Physics.Player.Id = @s Physics.Player.Id
    ...
    scoreboard players set #Physics.HitboxKillSuccess Physics 0
    execute if score #Physics.EntityInteractionRange Physics matches 0..3072 as @e[type=minecraft:interaction,predicate=physics:same_player_id,distance=..11.8602540378,limit=1] run function physics:zprivate/punchable_hitbox/kill/try_kill
    execute if score #Physics.EntityInteractionRange Physics matches 3073..5120 as @e[type=minecraft:interaction,predicate=physics:same_player_id,distance=..13.8602540378,limit=1] run function physics:zprivate/punchable_hitbox/kill/try_kill
